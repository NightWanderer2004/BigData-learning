...
def folium_top_x_preds_mapper(
    ...
    top_x_jsons = selected_json_dict_generator(
        json_df, geographic_element, preds_data[geographic_element].values
    )
    ...
    map.choropleth(
        geo_str=top_x_jsons,  # geographical information for the areas
        data=top_x_data[
            [geographic_element, data_column]
        ],  # the data to use to colour the areas
        columns=[geographic_element, data_column],  # values from the data to use
        key_on="feature.properties.%s"
        % geographic_element,  # the corresponding data in the geoJson to identify the areas
